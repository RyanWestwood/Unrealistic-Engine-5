...
namespace UE {
	...
	void GUIRenderer::DrawTexture(int x, int y, std::shared_ptr<Texture> texture)
	{
		...
		Vertex textQuad[] = {
			...
			Vertex(x, y + textureHeight, 0.0f, 0.0f, 0.0f),
			Vertex(x + textureWidth, y + textureHeight, 0.0f, 1.0f, 0.0f),

			Vertex(x + textureWidth, y + textureHeight, 0.0f, 1.0f, 0.0f),
			Vertex(x + textureWidth, y, 0.0f, 1.0f, 1.0f),
			Vertex(x, y, 0.0f, 0.0f, 1.0f),
		};
		...
		glVertexAttribPointer(m_RenderData.vertexLocation, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, x));
		glEnableVertexAttribArray(m_RenderData.vertexUVLocation);
		glVertexAttribPointer(m_RenderData.vertexUVLocation, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, u));
