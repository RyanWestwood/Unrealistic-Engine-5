...
namespace UE {
	...
	void BillboardRenderer::Init()
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(billboard), billboard.data(), GL_STATIC_DRAW);
	}
	...
	void BillboardRenderer::Draw()
	{
		...
		glVertexAttribPointer(vertexLocation, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, x)));

		glEnableVertexAttribArray(vertexUVLocation);
		glVertexAttribPointer(vertexUVLocation, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, u)));
