...
namespace UE {
	...
	void Billboard::Draw()
	{
		...
		glVertexAttribPointer(m_RenderData.vertexLocation, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, x)));

		glEnableVertexAttribArray(m_RenderData.vertexUVLocation);
		glVertexAttribPointer(m_RenderData.vertexUVLocation, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, u)));
