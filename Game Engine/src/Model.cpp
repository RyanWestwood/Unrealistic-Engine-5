...
void UE::Model::Init(const char* modelPath, const char* texturePath)
{
	m_Model = std::make_shared<Mesh>();
	bool result = m_Model->LoadFromFile((g_ModelDirectory + std::string(modelPath)).c_str());
	if (!result) std::cerr << "Failed to load model!\n";
...
void UE::Model::Draw(const std::shared_ptr<Camera>& camera) {
	...
	for (int MeshIdx = 0; MeshIdx < pScene->mNumMeshes; MeshIdx++)
	{
		...
		for (int faceIdx = 0; faceIdx < mesh->mNumFaces; faceIdx++)
		{
			...
			for (int vertexIdx = 0; vertexIdx < 3; vertexIdx++)
			{
				...
				m_Vertices.emplace_back(Vertex(pos->x, pos->y, pos->z, uv.x, uv.y));
			}
		}
	}
...
void UE::MeshRenderer::Draw(const std::shared_ptr<Camera>& camera)
{
	...
	glVertexAttribPointer(m_VertexPos3DLocation, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, x)));
	glEnableVertexAttribArray(m_VertexPos3DLocation);

	glVertexAttribPointer(m_VertexUVLocation, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<void*>(offsetof(Vertex, u)));
